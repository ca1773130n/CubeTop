...
enum enCameraType
{
	DXUT_CAM_TYPE_FP,	// 1ÀÎÄª Ä«¸Þ¶ó
	DXUT_CAM_TYPE_MV,	// ¸ðµ¨ºä Ä«¸Þ¶ó
};
...
public:
	...
	void SetFOV( FLOAT fFOV ) { m_fFOV = fFOV; SetProjParams( fFOV, m_fAspect, m_fNearPlane, m_fFarPlane ); }	
	void SetAspect( FLOAT fAspect ) { m_fAspect = fAspect; }
	void SetName( LPCWSTR name ) { StringCchCopy( m_lpszName, MAX_PATH, name ); }
	void SavePoseToDefault( void ) { m_vDefaultEye = m_vEye; m_vDefaultLookAt = m_vLookAt; m_fDefaultFOV = m_fFOV; }
    // Functions to get state
    const D3DXMATRIX*  GetViewMatrix() const { return &m_mView; }
	void  GetBillboardMatrix( D3DXMATRIX *pMat ) const { D3DXMatrixInverse( pMat, NULL, &m_mView ); pMat->_41 = pMat->_42 = pMat->_43 = 0; }
	...
	D3DXVECTOR3 GetDefaultEye() { return m_vDefaultEye; }
	D3DXVECTOR3 GetDefaultLookAt() { return m_vDefaultLookAt; }
	float GetDefaultFOV() { return m_fDefaultFOV; }

	LPCWSTR GetName() const { return m_lpszName; }
	enCameraType GetType() const { return m_enType; }
...
protected:
	...
	enCameraType		  m_enType;
	D3DXMATRIX            m_mWorld;             // World
...
public:
	...
	CD3DArcBall* GetArcBall( void ) { return &m_ViewArcBall; }
...
public:
	...
	HRESULT CalcTextBox( LPRECT rc, DWORD dwFlags, const WCHAR* strMsg );
	HRESULT DrawTextBox( LPRECT rc, DWORD dwFlags, const WCHAR* strMsg );
