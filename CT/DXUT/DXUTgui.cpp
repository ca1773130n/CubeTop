...
HRESULT CDXUTDialog::OnRender( float fElapsedTime, BOOL bDrawFrame/*=FALSE*/, UINT iFrameBorder/*=0*/ )
{
	...
	if( m_bCaption )
	{
		...
		RECT rc = { 0, 0, m_width, m_nCaptionHeight };
		WCHAR wszOutput[256];
	...
	if( !m_bMinimized )
	{
		...
		if( bDrawFrame )
		{
			...
			DrawControlFrame( &rc, iFrameBorder, fElapsedTime );
		}
...
// for CubeTop 튜닝
//////////////////////////////////////////////////////////////////////////
void CDXUTDialog::DrawControlFrame( RECT* rc, UINT iBorderSize, FLOAT fElapsedTime )
{
	m_pControlFrame->SetVisible( TRUE );
	m_pControlFrame->UpdateRects( rc->left, rc->top, rc->right - rc->left, rc->bottom - rc->top, iBorderSize );
	m_pControlFrame->Render( m_pManager->GetD3DDevice(), fElapsedTime );
	m_pControlFrame->SetVisible( FALSE );
}

void CDXUTDialog::DrawControlFrame( UINT x, UINT y, UINT radius, FLOAT fElapsedTime )
{
	m_pCircleControlFrame->SetVisible( TRUE );
	m_pCircleControlFrame->Render( m_pManager->GetD3DDevice(), x, y, radius, fElapsedTime );
	m_pCircleControlFrame->SetVisible( FALSE );
}
...
HRESULT CDXUTDialog::AddStatic( int ID, LPCWSTR strText, int x, int y, int width, int height, UINT iFont, D3DCOLOR color, DWORD dwTextFormat, int border, bool bIsDefault, CDXUTStatic** ppCreated )
{
	CDXUTStatic* pCreated;
	AddStatic( ID, strText, x, y, width, height, bIsDefault, &pCreated );

	pCreated->SetFrameBorder( border );
	pCreated->GetElement( 0 )->SetFont( iFont, color, dwTextFormat );

	if( ppCreated ) *ppCreated = pCreated;
...
HRESULT CDXUTDialog::AddImageButton( int ID, int x, int y, int width, int height, int spacing, int border, DXUT_IMAGEBUTTON_TYPE type, UINT nHotkey, bool bIsDefault, bool bOnOff, CDXUTButton** ppCreated )
{
	HRESULT hr = S_OK;

	CDXUTImageButton* pButton = new CDXUTImageButton( this, spacing, border );
	...
	pButton->SetHotkey( nHotkey );
	pButton->m_bIsDefault = bIsDefault;
	pButton->SetOnOff( bOnOff );

	RECT rc;
	switch( type )
	{
	case DXUT_IBT_CAMERA:
		{
			SetRect( &rc, 0, 115, 64, 179 );
			break;
		}
	case DXUT_IBT_SEARCH:
		{
			SetRect( &rc, 0, 179, 64, 243 );
			break;
		}
	case DXUT_IBT_CLIPBD:
		{
			SetRect( &rc, 0, 435, 64, 499 );
			break;
		}
	case DXUT_IBT_MTVIEW:
		{
			SetRect( &rc, 0, 371, 64, 435 );
			break;
		}
	case DXUT_IBT_SUBRGN:
		{
			SetRect( &rc, 0, 307, 64, 371 );
			break;
		}
	case DXUT_IBT_ARRANG:
		{
			SetRect( &rc, 0, 499, 64, 563 );
			break;
		}
	case DXUT_IBT_PRIMIT:
		{
			SetRect( &rc, 0, 627, 64, 691 );
			break;
		}
	case DXUT_IBT_CONFIG:
		{
			SetRect( &rc, 0, 563, 64, 627 );
			break;
		}
	case DXUT_IBT_REWIND:
		{
			SetRect( &rc, 256, 243, 320, 307 );
			break;
		}
	case DXUT_IBT_FOWARD:
		{
			SetRect( &rc, 448, 243, 512, 307 );
			break;
		}
	case DXUT_IBT_PLAYPS:
		{
			SetRect( &rc, 320, 243, 384, 307 );
			break;
		}
	case DXUT_IBT_UPDIRE:
		{
			SetRect( &rc, 64, 243, 128, 307 );
			break;
		}
	case DXUT_IBT_HMDIRE:
		{
			SetRect( &rc, 0, 243, 64, 307 );
			break;
		}
	case DXUT_IBT_TOGGLE:
		{
			SetRect( &rc, 128, 243, 192, 307 );
			break;
		}
	}

	pButton->SetTextureRect( &rc );
	pButton->GetElement(1)->SetTexture(0, &rc );

	return S_OK;
}

HRESULT CDXUTDialog::AddCircleImageButton( int ID, int x, int y, int radius, int spacing, int border, DXUT_IMAGEBUTTON_TYPE type, UINT nHotkey, bool bIsDefault, bool bOnOff, CDXUTCircleImageButton** ppCreated )
{
	HRESULT hr = S_OK;

	CDXUTCircleImageButton* pButton = new CDXUTCircleImageButton( this, x, y, radius, spacing, border );
	...
	pButton->SetID( ID ); 
	pButton->SetLocation( x, y );
	pButton->SetRadius( radius );
	pButton->SetSize( radius * 2, radius * 2 );
	pButton->SetHotkey( nHotkey );
	pButton->m_bIsDefault = bIsDefault;
	pButton->SetOnOff( bOnOff );
	pButton->SetSpacing( spacing );
	RECT rc;
	switch( type )
	{
	case DXUT_IBT_SEARCH:
	...
	case DXUT_CIBT_PILING:
		{
			SetRect( &rc, 0, 691, 64, 755 );
			break;
		}
	case DXUT_CIBT_PILINGBYTYPES:
		{
			SetRect( &rc, 64, 691, 128, 755 );
			break;
		}
	case DXUT_CIBT_LOCK:
		{
			SetRect( &rc, 128, 691, 192, 755 );
			break;
		}
	case DXUT_CIBT_UNLOCK:
		{
			SetRect( &rc, 192, 691, 256, 755 );
			break;
		}
	case DXUT_CIBT_LINK:
		{
			SetRect( &rc, 256, 691, 320, 755 );
			break;
		}
	case DXUT_CIBT_UNLINK:
		{
			SetRect( &rc, 320, 691, 384, 755 );
			break;
		}
	case DXUT_CIBT_COPY:
		{
			SetRect( &rc, 384, 691, 448, 755 );
			break;
		}
	case DXUT_CIBT_CUT:
		{
			SetRect( &rc, 448, 691, 512, 755 );
			break;
		}
	case DXUT_CIBT_DELETE:
		{
			SetRect( &rc, 512, 691, 576, 755 );
			break;
		}
	case DXUT_CIBT_CM_BACKGROUND:
		{
			SetRect( &rc, 436, 28, 523, 115 );
			break;
		}
	case DXUT_IBT_SUBRGN:
		{
	...
	case DXUT_CIBT_BREAKPILE:
		{
			SetRect( &rc, 0, 755, 64, 819 );
			break;
		}
	case DXUT_CIBT_MESH:
		{
	...
	pButton->SetTextureRect( &rc );
	pButton->GetElement(0)->SetTexture(0, &rc );

	return S_OK;
}

HRESULT CDXUTDialog::AddContextMenu( int ID, LPCWSTR strText, UINT nHotkey, bool bIsDefault, CDXUTContextMenu** ppCreated )
{
	HRESULT hr = S_OK;

	CDXUTContextMenu* pCM = new CDXUTContextMenu( this, strText );
	...
	pCM->SetLocation( 0, 0 );
	pCM->SetSize( 0, 0 );
	pCM->SetHotkey( nHotkey );
	pCM->m_bIsDefault = bIsDefault;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddComboBox( int ID, int x, int y, int width, int height, UINT border, UINT nHotKey/*=0*/, bool bIsDefault/*=false*/, CDXUTComboBox** ppCreated/*=NULL */ )
{
    HRESULT hr = S_OK;
	...
	pComboBox->SetFrameBorder( border );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddListBox( int ID, int x, int y, int width, int height, int border, DWORD dwStyle/*=0*/, CDXUTListBox** ppCreated/*=NULL */ )
{
    HRESULT hr = S_OK;
	...
	pListBox->SetFrameBorder( border );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddTextBox( int ID, int x, int y, int width, int height, int spacing, int border, LPCWSTR lpszStr, D3DCOLOR textColor, DWORD dwTextFormat, CDXUTTextBox** ppCreated )
{
	HRESULT hr = S_OK;
	CDXUTTextBox *pTextBox = new CDXUTTextBox( this );
	...
	hr = AddControl( pTextBox );
	if( FAILED(hr) )
		return hr;

	// Set the ID and position
	pTextBox->SetID( ID );
	pTextBox->SetLocation( x, y );
	pTextBox->SetText( lpszStr );
	pTextBox->SetStyle( spacing, textColor, dwTextFormat );
	pTextBox->SetFrameBorder( border );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddArcBall( int ID, int x, int y, int width, int height, CDXUTArcBall** ppCreated )
{
	HRESULT hr = S_OK;
	CDXUTArcBall* pArcBall = new CDXUTArcBall( this );
	...
	hr = AddControl( pArcBall );
	if( FAILED(hr) )
		return hr;

	// Set the ID and position
	pArcBall->SetID( ID );
	pArcBall->SetLocation( x, y );
	pArcBall->SetSize( width, height );

	return S_OK;}

//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddColorBox( int ID, int x, int y, int width, int height, D3DCOLOR color, CDXUTColorBox** ppCreated )
{
	HRESULT hr = S_OK;
	CDXUTColorBox* pColorBox = new CDXUTColorBox( this );
	...
	hr = AddControl( pColorBox );
	if( FAILED(hr) )
		return hr;

	// Set the ID and position
	pColorBox->SetID( ID );
	pColorBox->SetLocation( x, y );
	pColorBox->SetSize( width, height );
...
HRESULT CDXUTDialog::DrawPolyCircle( int x, int y, int LineCount, float fScale, D3DCOLOR color )
{
	LPDIRECT3DDEVICE9 pd3dDevice = m_pManager->GetD3DDevice();
	...
	DXUT_SCREEN_VERTEX* vertices = new DXUT_SCREEN_VERTEX[ LineCount + 2 ];
		
	for( UINT i=1; i < LineCount + 2; i++ )
    {
        vertices[i].x = x + fScale * cos(fRotationAngleAverage * (i - 1));
        vertices[i].y = y + fScale * sin(fRotationAngleAverage * (i - 1));
        vertices[i].z = 0.5f;
        vertices[i].h = 1.0f;
	...
	m_pManager->m_pSprite->Flush();
	IDirect3DVertexDeclaration9 *pDecl = NULL;
	pd3dDevice->GetVertexDeclaration( &pDecl );  
	pd3dDevice->SetFVF( DXUT_SCREEN_VERTEX::FVF );
	...
	pd3dDevice->DrawPrimitiveUP( D3DPT_TRIANGLEFAN, LineCount , vertices, sizeof(DXUT_SCREEN_VERTEX) );
...
HRESULT CDXUTDialog::DrawText( LPCWSTR strText, CDXUTElement* pElement, RECT* prcDest, bool bShadow, int nCount )
{
    ...
    if( bShadow )
    {
        ...
        hr = pFontNode->pFont->DrawText( m_pManager->m_pSprite, strText, nCount, &rcShadow, pElement->dwTextFormat, D3DCOLOR_ARGB(DWORD(pElement->FontColor.Current.a * 128), 0, 0, 0) );
        if( FAILED(hr) )
            return hr;
    }
...
void CDXUTDialog::InitDefaultElements()
{
    ...
    //-------------------------------------
    SetRect( &rcTexture, 292, 28, 372, 108 );
    m_CapElement.SetTexture( 0, &rcTexture );
    m_CapElement.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
    m_CapElement.FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(255, 0, 0, 0);
    m_CapElement.SetFont( 0, D3DCOLOR_ARGB(255, 0, 0, 0), DT_CENTER | DT_VCENTER );
    ...
    //-------------------------------------
    Element.SetFont( 0 );
	Element.FontColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB( 200, 0, 0, 0 );
	...
	//-------------------------------------
	// CDXUTImageButton - Button
	//-------------------------------------
	SetRect( &rcTexture, 292, 28, 372, 108 );
	Element.SetTexture( 0, &rcTexture );
	Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_PRESSED ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA_HOVERED, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_MOUSEOVER ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA_HIGHLIGHTED, 255, 255, 255);
	SetDefaultElement( DXUT_CONTROL_IMAGEBUTTON, 0, &Element );

	/*
	SetRect( &rcTexture, 1, 1, 1, 1 );
	Element.SetTexture( 0, &rcTexture );
	Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_PRESSED ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA_HOVERED, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_MOUSEOVER ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA_HIGHLIGHTED, 255, 255, 255);
	SetDefaultElement( DXUT_CONTROL_IMAGEBUTTON, 1, &Element );
	*/

	SetRect( &rcTexture, 1, 1, 1, 1 );
	Element.SetTexture( 0, &rcTexture );
	Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(255, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_PRESSED ] = D3DCOLOR_ARGB(240, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_MOUSEOVER ] = D3DCOLOR_ARGB(250, 255, 255, 255);
	SetDefaultElement( DXUT_CONTROL_IMAGEBUTTON, 1, &Element );

	//-------------------------------------
	// CDXUTCircleImageButton - Button
	//-------------------------------------
	SetRect( &rcTexture, 1, 1, 1, 1 );
	Element.SetTexture( 0, &rcTexture );
	/*
	Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(220, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_PRESSED ] = D3DCOLOR_ARGB(255, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_MOUSEOVER ] = D3DCOLOR_ARGB(250, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_HIDDEN ] = D3DCOLOR_ARGB(0, 255, 255, 255);
	*/
	Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(200, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_PRESSED ] = D3DCOLOR_ARGB(240, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB(230, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_MOUSEOVER ] = D3DCOLOR_ARGB(250, 255, 255, 255);
	SetDefaultElement( DXUT_CONTROL_CIRCLEIMAGEBUTTON, 0, &Element );

    //-------------------------------------
    // CDXUTCheckBox - Box
    //-------------------------------------
    SetRect( &rcTexture, 879, 20, 906, 47 );
    Element.SetTexture( 0, &rcTexture );
    ...
    // CDXUTCheckBox - Check
    //-------------------------------------
    SetRect( &rcTexture, 906, 20, 933, 47 );
    Element.SetTexture( 0, &rcTexture );
    ...
    //-------------------------------------
    SetRect( &rcTexture, 933, 20, 950, 47 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0, D3DCOLOR_ARGB(255, 255, 255, 255), DT_LEFT | DT_VCENTER );
    Element.FontColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB( 200, 200, 200, 200 );
    Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(150, 255, 255, 255);
    ...
    //-------------------------------------
    SetRect( &rcTexture, 960, 20, 987, 47 );
    Element.SetTexture( 0, &rcTexture );
    ...
    // CDXUTComboBox - Main
    //-------------------------------------
    SetRect( &rcTexture, 21, 21, 271, 94 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0 );
    Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 200, 200, 200);
    Element.TextureColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA_HOVERED, 230, 230, 230);
    ...
    //-------------------------------------
    SetRect( &rcTexture, 838, 20, 864, 46 );
    Element.SetTexture( 0, &rcTexture );
    Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
    Element.TextureColor.States[ DXUT_STATE_PRESSED ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA_HIGHLIGHTED, 150, 150, 150);
    Element.TextureColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA_HOVERED, 255, 255, 255);
    ...
    //-------------------------------------
    SetRect( &rcTexture, 759, 0, 987, 20 );
    Element.SetTexture( 0, &rcTexture );
    ...
    // CDXUTSlider - Track
    //-------------------------------------
    SetRect( &rcTexture, 292, 0, 759, 28 );
    Element.SetTexture( 0, &rcTexture );
    Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(150, 255, 255, 255);
    Element.TextureColor.States[ DXUT_STATE_FOCUS ] = D3DCOLOR_ARGB(200, 255, 255, 255);
    Element.TextureColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB(70, 255, 255, 255);
    ...
    // CDXUTSlider - Button
    //-------------------------------------
    SetRect( &rcTexture, 864, 20, 879, 35 );
    Element.SetTexture( 0, &rcTexture );
    ...
    //-------------------------------------
    SetRect( &rcTexture, 987, 0, 1009, 21 );
    Element.SetTexture( 0, &rcTexture );
    Element.TextureColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB(255, 200, 200, 200);
	Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
    ...
    //-------------------------------------    
	SetRect( &rcTexture, 987, 21, 1009, 42 );
    Element.SetTexture( 0, &rcTexture );
    Element.TextureColor.States[ DXUT_STATE_DISABLED ] = D3DCOLOR_ARGB(255, 200, 200, 200);
	Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
    ...
    //-------------------------------------
    SetRect( &rcTexture, 1009, 0, 1024, 458 );
    Element.SetTexture( 0, &rcTexture );	
	Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
	...
	// 컨트롤 프레임
	//////////////////////////////////////////////////////////////////////////
	m_pControlFrame = new CDXUTControlFrame( this );	

	CDXUTElement* pElement = new CDXUTElement[9];

	SetRect( &rcTexture, 21, 21, 271, 94 );
	pElement[0].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 0, &pElement[0] );
	SetRect( &rcTexture, 0, 0, 21, 21 );
	pElement[1].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 1, &pElement[1] );
	SetRect( &rcTexture, 21, 0, 271, 21 );
	pElement[2].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 2, &pElement[2] );
	SetRect( &rcTexture, 271, 0, 292, 21 );
	pElement[3].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 3, &pElement[3] );
	SetRect( &rcTexture, 0, 21, 21, 94 );
	pElement[4].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 4, &pElement[4] );
	SetRect( &rcTexture, 271, 21, 292, 94 );
	pElement[5].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 5, &pElement[5] );
	SetRect( &rcTexture, 0, 94, 21, 115 );
	pElement[6].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 6, &pElement[6] );
	SetRect( &rcTexture, 21, 94, 271, 115 );
	pElement[7].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 7, &pElement[7] );
	SetRect( &rcTexture, 271, 94, 292, 115 );
	pElement[8].SetTexture( 0, &rcTexture );
	m_pControlFrame->AddElement( 8, &pElement[8] );

	for( int i=0; i < 9; i++ ) 
	{
		pElement[i].TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
		pElement[i].TextureColor.States[ DXUT_STATE_HIDDEN ] = D3DCOLOR_ARGB(0, 0, 0, 0);
	}

	m_pControlFrame->SetVisible( FALSE );
	AddControl( (CDXUTControl*)m_pControlFrame );

	m_pCircleControlFrame = new CDXUTCircleControlFrame( this );
	pElement = new CDXUTElement;
	SetRect( &rcTexture, 524, 29, 586, 91 );
	m_pCircleControlFrame->AddElement( 0, pElement );
	pElement->SetTexture( 0, &rcTexture );
	pElement->TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(CUSTOMUI_DEFAULT_ALPHA, 255, 255, 255);
	pElement->TextureColor.States[ DXUT_STATE_HIDDEN ] = D3DCOLOR_ARGB(0, 255, 255, 255);
	m_pCircleControlFrame->SetVisible( FALSE );
	AddControl( (CDXUTControl*)m_pCircleControlFrame );
	...
	Element.SetTexture( 0, &rcTexture );
	Element.SetFont( 0, D3DCOLOR_ARGB( 200, 0, 0, 0 ), DT_LEFT|DT_NOCLIP|DT_WORDBREAK );
	SetDefaultElement( DXUT_CONTROL_TEXTBOX, 0, &Element );

	//////////////////////////////////////////////////////////////////////////
	// 아크볼
	//////////////////////////////////////////////////////////////////////////
	SetRect( &rcTexture, 448, 115, 568, 235 );
	Element.SetTexture( 0, &rcTexture );
	SetDefaultElement( DXUT_CONTROL_ARCBALL, 0, &Element );

	//////////////////////////////////////////////////////////////////////////
	// 컬러박스
	//////////////////////////////////////////////////////////////////////////
	SetDefaultElement( DXUT_CONTROL_COLORBOX, 0, &Element );	
}
...
{    
	...
	m_pDialog->DrawControlFrame( &m_rcBoundingBox, m_iBorder, fElapsedTime );
        
    CDXUTElement* pElement = m_Elements.GetAt( 0 );

    pElement->FontColor.Blend( iState, fElapsedTime );
    
    m_pDialog->DrawText( m_strText, pElement, &m_rcBoundingBox, FALSE );
}
...
//--------------------------------------------------------------------------------------
void CDXUTListBox::UnSelectAll( VOID )
{
	DXUTListBoxItem** pItems = m_Items.GetData();
	for( DWORD i=0; i < m_Items.GetSize(); i++ )
		pItems[i]->bSelected = FALSE;
}
...
CDXUTEditBox::CDXUTEditBox( CDXUTDialog *pDialog )
{
    ...
    m_nBorder = 21;  // Default border width
...
CDXUTControlFrame::CDXUTControlFrame( CDXUTDialog* pDialog /* = NULL */ )
{
	m_pDialog = pDialog;	
}

void CDXUTControlFrame::UpdateRects( int x, int y, int width, int height , UINT border )
{
	RECT inner, outer;

	m_rcRender[0].left = x;
	m_rcRender[0].top = y;
	m_rcRender[0].right = x + width;
	m_rcRender[0].bottom = y + height;

	// Update the text rectangle
	inner = outer = m_rcRender[0];

	// First inflate by m_nBorder to compute render rects
	InflateRect( &outer, border, border );

	// Update the render rectangles
	SetRect( &m_rcRender[1], outer.left, outer.top, inner.left, inner.top  );
	SetRect( &m_rcRender[2], inner.left, outer.top, inner.right, inner.top );
	SetRect( &m_rcRender[3], inner.right, outer.top, outer.right, inner.top );
	SetRect( &m_rcRender[4], outer.left, inner.top, inner.left, inner.bottom );
	SetRect( &m_rcRender[5], inner.right, inner.top, outer.right, inner.bottom );
	SetRect( &m_rcRender[6], outer.left, inner.bottom, inner.left, outer.bottom );
	SetRect( &m_rcRender[7], inner.left, inner.bottom, inner.right, outer.bottom );
	SetRect( &m_rcRender[8], inner.right, inner.bottom, outer.right, outer.bottom );
}

void CDXUTControlFrame::AddElement( UINT iElement, CDXUTElement* pElement )
{
	m_Elements.Add( pElement );
}

void CDXUTControlFrame::Render( IDirect3DDevice9* pd3dDevice, FLOAT fElapsedTime )
{
	...
	{
		pElement = m_Elements.GetAt( e );
		pElement->TextureColor.Blend( DXUT_STATE_NORMAL, fElapsedTime, 0.1f );

		m_pDialog->DrawSprite( pElement, &m_rcRender[e] );
	}
}

CDXUTCircleControlFrame::CDXUTCircleControlFrame( CDXUTDialog* pDialog /* = NULL */ )
{
	m_pDialog = pDialog;
}

void CDXUTCircleControlFrame::AddElement( UINT iElement, CDXUTElement* pElement )
{
	m_Elements.Add( pElement );
}

void CDXUTCircleControlFrame::Render( IDirect3DDevice9* pd3dDevice, UINT x, UINT y, UINT radius, FLOAT fElapsedTime )
{
	if( m_bVisible == FALSE ) return;
	...
	pElement = m_Elements.GetAt( 0 );
	pElement->TextureColor.Blend( DXUT_STATE_NORMAL, fElapsedTime, 0.008f );

	m_pDialog->DrawSprite( pElement, &rc );
}

CDXUTTextBox::CDXUTTextBox( CDXUTDialog* pDialog /* = NULL */ )
{
	m_Type = DXUT_CONTROL_TEXTBOX;
	m_pDialog = pDialog;
	m_lpszStr = L"";
}

VOID
CDXUTTextBox::Render( IDirect3DDevice9* pd3dDevice, FLOAT fElapsedTime )
{
	if( m_bVisible == FALSE ) return;

	HRESULT hr;

	m_pDialog->DrawControlFrame( &m_rcBounding, m_iBorder, fElapsedTime );

	CDXUTElement* pElement = m_Elements.GetAt( 0 );
	pElement->TextureColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
	pElement->FontColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
	m_pDialog->DrawText( m_lpszStr, pElement, &m_rcText, FALSE );
}

VOID CDXUTTextBox::SetStyle( UINT iSpacing, D3DCOLOR textColor, DWORD dwTextFormat )
{
	m_dwTextFormat = dwTextFormat;
	m_iSpacing = iSpacing;
	m_textColor = textColor;
	m_Elements.GetAt( 0 )->SetFont( 0, D3DCOLOR_ARGB( 200, 0, 0, 0 ), dwTextFormat );
}

VOID
CDXUTTextBox::UpdateRects( VOID )
{
	SetRect( &m_rcBounding, m_x, m_y, m_x + m_width, m_y + m_height );
	m_rcText = m_rcBounding;
	InflateRect( &m_rcText, -m_iSpacing, -m_iSpacing );		
}

VOID
CDXUTTextBox::SetText( LPCWSTR lpszStr )
{	
	m_lpszStr = lpszStr;
}

CDXUTArcBall::CDXUTArcBall( CDXUTDialog* pDialog /* = NULL */ )
{
	m_Type = DXUT_CONTROL_ARCBALL;
	m_pDialog = pDialog;
}

VOID
CDXUTArcBall::Render( IDirect3DDevice9* pd3dDevice, FLOAT fElapsedTime )
{
	if( m_bVisible == FALSE ) return;
...
bool
CDXUTArcBall::HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam )
{
	return TRUE;
	//return GetCTmain()->m_pCurrentCam->GetArcBall()->HandleMessages( AfxGetMainWnd()->m_hWnd, uMsg, wParam, lParam );
}

bool
CDXUTArcBall::MsgProc( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	return m_ArcBall.HandleMessages( AfxGetMainWnd()->m_hWnd, uMsg, wParam, lParam );
}

CDXUTColorBox::CDXUTColorBox( CDXUTDialog* pDialog /* = NULL */ )
{
	m_pDialog = pDialog;
}

VOID
CDXUTColorBox::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
	RECT rc;
	m_pDialog->DrawRect( &m_rcBoundingBox, m_color );
}

CDXUTImageButton::CDXUTImageButton( CDXUTDialog *pDialog /* = NULL */, UINT iSpacing, UINT iBorder )
{
	m_Type = DXUT_CONTROL_IMAGEBUTTON;
	m_pDialog = pDialog;
...
VOID
CDXUTImageButton::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
	DXUT_CONTROL_STATE iState = DXUT_STATE_NORMAL;
	...
	CDXUTElement* pElement = m_Elements.GetAt( 0 );
	float fBlendRate = ( iState == DXUT_STATE_PRESSED ) ? 0.0f : 0.2f;

	RECT rcWindow = m_rcBoundingBox;

	m_pDialog->DrawControlFrame( &rcWindow, m_iBorder, fElapsedTime );
	...
	InflateRect( &rcWindow, -m_iSpacing, -m_iSpacing );
	m_pDialog->DrawSprite( pElement, &rcWindow);
}

VOID
CDXUTImageButton::SetTextureRect( RECT* rc )
{
	m_rcTexture = *rc;
}

bool
CDXUTImageButton::HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam )
{
	switch( uMsg )
	{
	case WM_LBUTTONUP:
		{
			...
			{
				m_bIsOn = !m_bIsOn;
				UINT width = m_rcTexture.right - m_rcTexture.left;
				if( !m_bIsOn )
				{
					m_rcTexture.left += width;
					m_rcTexture.right += width;
				}
				else 
				{
					m_rcTexture.left -= width;
...
CDXUTCircleImageButton::CDXUTCircleImageButton( CDXUTDialog* pDialog /* = NULL */, UINT x, UINT y, UINT radius, UINT iSpacing /* = 21 */, UINT iBorder /* = 21 */ )
{
	m_pDialog = pDialog;
	m_ptCenter.x = x;
	m_ptCenter.y = y;
...
BOOL CDXUTCircleImageButton::ContainsPoint( POINT pt )
{
	D3DXVECTOR3 vDiff( m_x - pt.x, m_y - pt.y, 0 );

	if( m_iRadius > D3DXVec3Length(&vDiff) ) 
		return TRUE;
...
void
CDXUTCircleImageButton::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
	DXUT_CONTROL_STATE iState = DXUT_STATE_NORMAL;
	...
	{
		iState = DXUT_STATE_MOUSEOVER;
		m_pDialog->DrawControlFrame( m_x, m_y, m_iRadius, fElapsedTime );
	}
	else if( m_bHasFocus )
	{
		iState = DXUT_STATE_FOCUS;
	...
	RECT rcWindow;
	SetRect( &rcWindow, m_x - m_iRadius, m_y - m_iRadius, m_x + m_iRadius, m_y + m_iRadius );

	

	CDXUTElement* pElement = m_Elements.GetAt( 0 );

	float fBlendRate = ( iState == DXUT_STATE_PRESSED ) ? 0.0f : 0.1f;
	pElement->TextureColor.Blend( iState, fElapsedTime, fBlendRate );
 ...
 * \brief 생성자
 ...
 컨텍스트 메뉴 객체 생성시에 수행할 작업을 작성.
 
 * \param pDialog	CDXUTDialog 포인터
 * \param strName	컨텍스트 메뉴 이름
 * \return			없음
*/
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDXUTContextMenu::CDXUTContextMenu( CDXUTDialog* pDialog /* = NULL */, CString strName )
{
	m_bVisible		= FALSE;
	m_fTime			= 0;
	m_pDialog		= pDialog;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \brief 컨텍스트 메뉴를 보여주도록 설정하는 함수
 
 컨텍스트 메뉴의 컨트롤들을 보이게 설정하고 버튼을의 초기 위치를 지정한다.
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID
CDXUTContextMenu::Show( POINT pt )
{
	m_fTime = 0;
	m_ptCenter = pt;
	m_bVisible = TRUE;

	DWORD nItem = m_Controls.GetSize();
	CDXUTControl** pControls = m_Controls.GetData();
	CDXUTCircleImageButton* pButton;

	// 초기 위치를 마우스 클릭 지점으로
	for( DWORD i=0; i < nItem; i++ )
	{
		pButton = (CDXUTCircleImageButton*)pControls[i];

		// theta를 배분
		pButton->SetTheta( (2 * PI / nItem) * i );
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \brief 컨텍스트 메뉴를 감추는 함수
 
 보여줄 때와 반대로 서서히 메뉴를 모으면서 감춘다.
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID
CDXUTContextMenu::Hide( VOID )
{
	// Show의 반대로 서서히 모으면서 페이드아웃
	DWORD nItem = m_Controls.GetSize();
	CDXUTControl** pControls = m_Controls.GetData();

	// 초기 위치를 마우스 클릭 지점으로
	for( DWORD i=0; i < nItem; i++ )
		pControls[i]->SetVisible( FALSE );
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * \brief 컨텍스트 메뉴를 그리는 함수
 
 메뉴의 배경 원을 그리고 원 둘레를 따라 버튼들을 위치시키면서 그려준다.
 
 * \param fElapsedTime	경과 시간
 * \return				애니메이션이 끝났는지 여부를 반환
*/
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID
CDXUTContextMenu::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
	...
	// 반지름을 정함
	UINT radiusButton = pControls[1]->GetWidth() / 2.f;
	
	if( m_bVisible ) m_fRadius = m_fTime * 360;
	else m_fRadius -= fElapsedTime * 360;
	
	if( m_fRadius > (nItem * radiusButton) / PI)
	{
		m_fRadius = (nItem * radiusButton) / PI;		
		bFinished = TRUE;
	}
	if( m_fRadius < 1 ) 
	{
	...
	if( !bHidden )
	{		
		int alpha = m_fRadius / ((nItem * radiusButton) / PI) * 200;
		int numTri = m_fRadius * 10;
		m_pDialog->DrawPolyCircle( m_ptCenter.x, m_ptCenter.y, numTri, m_fRadius + radiusButton * 1.5f, D3DCOLOR_ARGB(alpha, 0, 0, 0) );	
		m_pDialog->DrawPolyCircle( m_ptCenter.x, m_ptCenter.y, numTri, m_fRadius, D3DCOLOR_ARGB(alpha, 200, 200, 200) );	
	}
	
	// 메뉴 이름 
	//pControls[1]->SetLocation( m_ptCenter.x - pControls[1]->m_width / 2.f, m_ptCenter.y - pControls[1]->m_height * 2.f );

	
	// 원 둘레로 위치시킴
	FLOAT theta = 0;
	D3DXVECTOR2 v0( 0, (int)m_fRadius ), vPos;
	CDXUTCircleImageButton* pButton;
	...
	if( !bFinished ) 
	{
		for( DWORD i=0; i < nItem; i++ )
		{
			...
			// 현재 위치에서 theta를 구한다
			vPos.x = pButton->m_x - m_ptCenter.x;
			vPos.y = pButton->m_y - m_ptCenter.y;

			// theta를 증가시키면서 회전
			theta = pButton->GetTheta() + 0.1f;
			pButton->SetTheta( theta );

			vPos.x = m_ptCenter.x + cos( theta ) * v0.x - sin( theta ) * v0.y;
			vPos.y = m_ptCenter.y + sin( theta ) * v0.x + cos( theta ) * v0.y;

			pButton->SetLocation( vPos.x, vPos.y );
		}
	}	
}
