	...
	if(*bind==0) {
		...
		*/
		...
		if (tilemode) {
			...
			tilerect= (UINT*)MEM_mallocN(rectw*recth*sizeof(*tilerect), "tilerect");
			for (y=0; y<recth; y++) {
		...
		if (!is_pow2_limit(rectw) || !is_pow2_limit(recth)) {
			...
			scalerect= (UINT*)MEM_mallocN(rectw*recth*sizeof(*scalerect), "scalerect");
...
void update_realtime_image(BLImage *ima, int x, int y, int w, int h)
{
...
void free_realtime_image(BLImage *ima)
{
	if(ima->bindcode) {
...
void free_all_realtime_images(void)
{
	...
	for(ima=(BLImage*)G.main->image.first; ima; ima=(BLImage*)ima->id.next)
		free_realtime_image(ima);
}
...
void make_repbind(BLImage *ima)
{
	...
	if(ima->totbind>1) {
		ima->repbind= (UINT*)MEM_callocN(sizeof(int)*ima->totbind, "repbind");
	}
}

void update_realtime_textures()
{
	BLImage *ima = (BLImage*)G.main->image.first;
	while(ima) {
...
{
	struct MeEh *data = (struct MeEh*)userData;
...
static int draw_em_tf_mapped__set_draw(void *userData, int index)
{
	...
	tface = (MTFace*)CustomData_em_get(&em->fdata, efa->data, CD_MTFACE);
