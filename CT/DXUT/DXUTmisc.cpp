				...
				case CAM_MOVE_BACKWARD:
					{
						SetDrag( TRUE, 0.5f );
						m_fMoveScaler += 0.1f;
						SetScalers( 0.01f, m_fMoveScaler );
						break;
					}					
				}
...
VOID CFirstPersonCamera::FrameMove( FLOAT fElapsedTime )
{
	...
	if( m_fMoveScaler != 10.f && !(IsKeyDown(m_aKeys[CAM_MOVE_UP]) || IsKeyDown(m_aKeys[CAM_MOVE_DOWN]) || IsKeyDown(m_aKeys[CAM_MOVE_FORWARD]) || IsKeyDown(m_aKeys[CAM_MOVE_BACKWARD]) || IsKeyDown(m_aKeys[CAM_STRAFE_LEFT]) || IsKeyDown(m_aKeys[CAM_STRAFE_RIGHT])) ) 
	{
		m_fMoveScaler *= 0.5f;
		if( m_fMoveScaler < 10.f ) m_fMoveScaler = 10.f;
	}
        ...
        m_vGamePadRightThumb.z != 0 )
    {
        ...
        m_fCameraPitchAngle += fPitchDelta * m_fFOV * 4 / PI;
        m_fCameraYawAngle   += fYawDelta * m_fFOV * 4 / PI;
...
HRESULT CDXUTTextHelper::CalcTextBox( LPRECT rc, DWORD dwFlags, const WCHAR* strMsg )
{
	if( NULL == m_pFont ) 
		return DXUT_ERR_MSGBOX( L"DrawTextLine", E_INVALIDARG );
	...
	hr = m_pFont->DrawText( m_pSprite, strMsg, -1, rc, dwFlags|DT_CALCRECT, m_clr );
...
HRESULT CDXUTTextHelper::DrawTextBox( LPRECT rc, DWORD dwFlags, const WCHAR* strMsg )
{
	if( NULL == m_pFont ) 
		return DXUT_ERR_MSGBOX( L"DrawTextLine", E_INVALIDARG );
	...
	// ±×¸²ÀÚ
	hr = m_pFont->DrawText( m_pSprite, strMsg, -1, rc2, dwFlags, D3DCOLOR_XRGB( 0, 0, 0 ) );
...
LPD3DXMESH        CDXUTDirectionWidget::s_pMesh = NULL;    
...
HRESULT CDXUTDirectionWidget::StaticOnCreateDevice( IDirect3DDevice9* pd3dDevice )
{
    ...
    // Load the mesh with D3DX and get back a LPD3DXMESH.  For this
