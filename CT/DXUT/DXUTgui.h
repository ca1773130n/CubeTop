...
//
//////////////////////////////////////////////////////////////////////////
#define CUSTOMUI_DEFAULT_ALPHA				100
#define CUSTOMUI_DEFAULT_ALPHA_HOVERED		120
#define CUSTOMUI_DEFAULT_ALPHA_HIGHLIGHTED	150
...
class CDXUTCircleImageButton;
class CDXUTArcBall;
class CDXUTColorBox;
class CDXUTContextMenu;

// 컨트롤 프레임 
class CDXUTControlFrame;
class CDXUTCircleControlFrame;
...
{ 
    DXUT_CONTROL_BUTTON, 
	DXUT_CONTROL_IMAGEBUTTON,
	DXUT_CONTROL_CIRCLEIMAGEBUTTON,
    DXUT_CONTROL_STATIC, 
	...
	DXUT_CONTROL_FRAME,
	DXUT_CONTROL_TEXTBOX,
	DXUT_CONTROL_ARCBALL,
	DXUT_CONTROL_COLORBOX,
};
...
enum DXUT_IMAGEBUTTON_TYPE
{
	...
	DXUT_CIBT_CM_BACKGROUND,
	DXUT_CIBT_PILING,
	DXUT_CIBT_PILINGBYTYPES,
	DXUT_CIBT_LOCK,
	DXUT_CIBT_UNLOCK,
	DXUT_CIBT_LINK,
	DXUT_CIBT_UNLINK,
...
public:
	...
	HRESULT AddStatic( int ID, LPCWSTR strText, int x, int y, int width, int height, UINT iFont, D3DCOLOR color, DWORD dwTextFormat, int border, bool bIsDefault, CDXUTStatic** ppCreated );
	...
	HRESULT AddImageButton( int ID, int x, int y, int width, int height, int spacing, int border, DXUT_IMAGEBUTTON_TYPE type, UINT nHotkey, bool bIsDefault, bool bOnOff, CDXUTButton** ppCreated );
	HRESULT AddCircleImageButton( int ID, int x, int y, int radius, int spacing, int border, DXUT_IMAGEBUTTON_TYPE type, UINT nHotkey, bool bIsDefault, bool bOnOff, CDXUTCircleImageButton** ppCreated );
	HRESULT AddContextMenu( int ID, LPCWSTR strText, UINT nHotkey, bool bIsDefault, CDXUTContextMenu** ppCreated );
	...
	HRESULT AddComboBox( int ID, int x, int y, int width, int height, UINT border, UINT nHotKey=0, bool bIsDefault=false, CDXUTComboBox** ppCreated=NULL );
	...
	HRESULT AddListBox( int ID, int x, int y, int width, int height, int border, DWORD dwStyle=0, CDXUTListBox** ppCreated=NULL );
	HRESULT AddTextBox( int ID, int x, int y, int width, int height, int spacing, int border, LPCWSTR lpszStr, D3DCOLOR textColor, DWORD dwTextFormat, CDXUTTextBox** ppCreated ); // 튜닝
	HRESULT AddArcBall( int ID, int x, int y, int width, int height, CDXUTArcBall** ppCreated );
	HRESULT AddColorBox( int ID, int x, int y, int width, int height, D3DCOLOR color, CDXUTColorBox** ppCreated );
	...
	CDXUTTextBox*	  GetTextBox( int ID ) { return (CDXUTTextBox*) GetControl( ID, DXUT_CONTROL_TEXTBOX ); }
	...
	HRESULT DrawPolyCircle( int x, int y, int LineCount, float fScale, D3DCOLOR color );
	...
	HRESULT OnRender( float fElapsedTime, BOOL bDrawFrame = FALSE, UINT iFrameBorder = 0 );
	...
	CDXUTControl* Focused( VOID ) { return m_pControlMouseOver; }
	
	//////////////////////////////////////////////////////////////////////////
	// for CubeTop 튜닝
	//////////////////////////////////////////////////////////////////////////
	void DrawControlFrame( RECT* rc, UINT iBorderSize,  FLOAT fElapsedTime );
	void DrawControlFrame( UINT x, UINT y, UINT radius, FLOAT fElapsedTime );
...
private:
	...
	// for CubeTop 튜닝
	//////////////////////////////////////////////////////////////////////////
	CDXUTControlFrame* m_pControlFrame;
	CDXUTCircleControlFrame* m_pCircleControlFrame;
};
...
public:
	...
	void SetSpacing( int spacing ) { m_iSpacing = spacing; }
	void SetFrameBorder( int border ) { m_iBorder = border; }
...
public:
	...
	CGrowableArray< DXUTListBoxItem* >* GetItemArray( VOID ) { return &m_Items; }
...
// for CubeTop 튜닝
//////////////////////////////////////////////////////////////////////////
class CDXUTControlFrame : public CDXUTControl
{

public:
	CDXUTControlFrame( CDXUTDialog* pDialog = NULL );
	void			UpdateRects( int x, int y, int width, int height , UINT border );
	void			AddElement( UINT iElement, CDXUTElement* pElement );
	virtual void	Render( IDirect3DDevice9* pd3dDevice, FLOAT fElapsedTime );
...
class CDXUTCircleControlFrame : public CDXUTControl
{
public:
	CDXUTCircleControlFrame( CDXUTDialog* pDialog = NULL );
	void			AddElement( UINT iElement, CDXUTElement* pElement );
	virtual void	Render( IDirect3DDevice9* pd3dDevice, UINT x, UINT y, UINT radius, FLOAT fElapsedTime );
...
class CDXUTTextBox : public CDXUTControl
{
public:
	CDXUTTextBox( CDXUTDialog* pDialog = NULL );
	virtual VOID	Render( IDirect3DDevice9* pd3dDevice, FLOAT fElapsedTime );
	VOID			SetStyle( UINT iSpacing, D3DCOLOR textColor, DWORD dwTextFormat );
	VOID			UpdateRects();
	VOID			SetText( LPCWSTR lpszStr );	
...
class CDXUTArcBall : public CDXUTControl
{
public:
	CDXUTArcBall( CDXUTDialog* pDialog = NULL );
	virtual VOID	Render( IDirect3DDevice9* pd3dDevice, FLOAT fElapsedTime );
	virtual bool    HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam );
	virtual bool    MsgProc( UINT uMsg, WPARAM wParam, LPARAM lParam );
...
class CDXUTColorBox : public CDXUTControl
{
protected:
	D3DCOLOR m_color;

public:
	CDXUTColorBox( CDXUTDialog* pDialog = NULL );
...
public:
	CDXUTImageButton( CDXUTDialog *pDialog = NULL, UINT iSpacing = 21, UINT iBorder = 21 );
	
	virtual void	Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime );
	void			SetTextureRect( RECT* rc );
	void			SetOnOff( bool bOnOff ) { m_bOnOff = bOnOff; }
	virtual bool	HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam );
};

class CDXUTCircleImageButton : public CDXUTImageButton
{
protected:
	POINT			m_ptCenter;
	UINT			m_iRadius;
...
public:
	CDXUTCircleImageButton( CDXUTDialog* pDialog = NULL, UINT x = 0, UINT y = 0, UINT radius = 0, UINT iSpacing = 21, UINT iBorder = 21 );

	virtual void	Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime );
	virtual BOOL	ContainsPoint( POINT pt );
	VOID			SetRadius( UINT radius ) { m_iRadius = radius; }
	FLOAT			GetTheta( VOID ) { return m_fTheta; }
	VOID			SetTheta( FLOAT theta ) { m_fTheta = theta; }
};

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
@brief ContextMenu 클래스

*/
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
class CDXUTContextMenu : public CDXUTControl
{
protected:
	CDXUTDialog*							m_pDialog;
	CGrowableArray<CDXUTControl*>			m_Controls;
...
public:
	CDXUTContextMenu( CDXUTDialog* pDialog = NULL, CString strName = L"" );

	POINT									GetCenter( VOID )						{ return m_ptCenter; }
	...
	VOID									AddControl( CDXUTControl* pControl )	{ m_Controls.Add( pControl ); }

	CGrowableArray<CDXUTControl*>&			GetControls( VOID )						{ return m_Controls; }
